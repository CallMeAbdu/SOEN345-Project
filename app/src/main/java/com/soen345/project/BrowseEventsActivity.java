...
public class BrowseEventsActivity extends AppCompatActivity {
    ...
    private void setupUserInfo() {
        ...
        if (isNullOrBlank(email)) {
            email = authService.getSignedInEmail();
        }
        if (isNullOrBlank(email)) {
            email = getString(R.string.auth_unknown_user);
        }
    ...
    private void setupFilterListeners() {
        // TASK-2.2.2 - category filter (live text input)
        ...
        // TASK-2.2.3 - location filter (live text input)
    ...
    // -------------------------------------------------------------------------
    // TASK-2.2.1 / 2.2.2 / 2.2.3 - Filters | TASK-2.2.4 - Dynamic UI update
    // -------------------------------------------------------------------------
    ...
    private void applyFiltersAndRender() {
        ...
        if (!filterCategory.isEmpty()) {
            ...
            for (Event e : filtered) {
                if (e.getCategory().toLowerCase(Locale.US).contains(lower)) {
                    catFiltered.add(e);
                }
            }
        ...
        if (!filterLocation.isEmpty()) {
            ...
            for (Event e : filtered) {
                if (e.getLocation().toLowerCase(Locale.US).contains(lower)) {
                    locFiltered.add(e);
                }
            }
    ...
    private void renderEvents(List<Event> events) {
        ...
        if (events == null || events.isEmpty()) {
            boolean filtersActive = filterDateFromMillis > 0L
                    || filterDateToMillis > 0L
    ...
    private void showDatePicker(boolean isFrom) {
        ...
        if (current > 0L) {
            initial.setTimeInMillis(current);
        }
        ...
        new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    String label = sdf.format(new Date(pickedMillis));
