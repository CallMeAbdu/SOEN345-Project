...
public class FirebaseEventRepository implements EventRepository {
    ...
    public FirebaseEventRepository() { this(FirebaseFirestore.getInstance()); }

    public FirebaseEventRepository(FirebaseFirestore firestore) {
        if (firestore == null) throw new IllegalArgumentException("firestore cannot be null");
        ...
        firestore.collection(EVENTS_COLLECTION)
                ...
                .addOnSuccessListener(querySnapshot -> {
                    ...
                    for (DocumentSnapshot snapshot : querySnapshot.getDocuments()) {
                        try { events.add(toEvent(snapshot)); } catch (Exception ignored) {}
                    }
                    sortByDate(events);
    ...
    public EventListenerHandle listenToEvents(EventListCallback callback) {
        if (callback == null) return () -> {};
        ListenerRegistration registration = firestore.collection(EVENTS_COLLECTION)
                .addSnapshotListener((querySnapshot, error) -> {
                    if (error != null) {
                        callback.onError(resolveErrorMessage(error, "Failed to load events."));
                        return;
                    }
    ...
    public void createEvent(Event event, EventActionCallback callback) {
        if (callback == null) return;
        if (event == null) { callback.onError("Event cannot be null."); return; }
        DocumentReference document = firestore.collection(EVENTS_COLLECTION).document();
        document.set(createEventWriteMap(document.getId(), event))
    ...
    public void updateEvent(Event event, EventActionCallback callback) {
        if (callback == null) return;
        if (event == null || isBlank(event.getDocumentId())) { callback.onError("Invalid event."); return; }
        firestore.collection(EVENTS_COLLECTION)
                .document(event.getDocumentId())
                .update(createEventWriteMap(
                        isBlank(event.getEventId()) ? event.getDocumentId() : event.getEventId(), event))
                .addOnSuccessListener(unused -> callback.onSuccess())
                .addOnFailureListener(e -> callback.onError(resolveErrorMessage(e, "Could not save event.")));
    }
    ...
    public void updateStatus(String documentId, EventStatus status, EventActionCallback callback) {
        if (callback == null) return;
        if (isBlank(documentId)) { callback.onError("Invalid event."); return; }
    ...
    private void sortByDate(List<Event> events) {
        Collections.sort(events, (a, b) -> Long.compare(b.getDateTimeMillis(), a.getDateTimeMillis()));
    }
    ...
    private Event toEvent(DocumentSnapshot snapshot) {
        ...
        if (isBlank(eventId)) eventId = documentId;
        ...
        if (dateTimeMillis <= 0L) dateTimeMillis = parseDateTimeString(safeString(dateTimeRaw));
        ...
        if (isBlank(statusRaw)) {
            ...
            statusRaw = cancelledLegacy != null && cancelledLegacy
                    ? EventStatus.CANCELLED.value() : EventStatus.ACTIVE.value();
        }
        ...
        if (capacityRemaining > capacityTotal && capacityTotal > 0) capacityRemaining = capacityTotal;
        if (capacityRemaining < 0) capacityRemaining = 0;

        return new Event(documentId, eventId, title, category, location,
                dateTimeMillis, status, capacityTotal, capacityRemaining);
    }
    ...
    private long parseDateTimeString(String value) {
        ...
        for (String pattern : DATE_TIME_PATTERNS) {
            ...
            try { Date d = format.parse(value); if (d != null) return d.getTime(); }
