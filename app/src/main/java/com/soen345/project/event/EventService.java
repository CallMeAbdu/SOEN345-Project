...
public class EventService {
    ...
    /** Returns a handle — call handle.remove() in onDestroy to stop listening. */
    public EventListenerHandle listenToEvents(EventListCallback callback) {
        return eventRepository.listenToEvents(callback);
    }

    public void createEvent(
            String title, String category, String location,
            long dateTimeMillis, int capacityTotal, int capacityRemaining,
            EventActionCallback callback
    ) {
        Event event = new Event("", "", trim(title), trim(category), trim(location),
                dateTimeMillis, EventStatus.ACTIVE, capacityTotal, capacityRemaining);
        eventRepository.createEvent(event, callback);
    }

    public void updateEvent(
            Event existingEvent, String title, String category, String location,
            long dateTimeMillis, int capacityTotal, int capacityRemaining,
            EventActionCallback callback
    ) {
        ...
        Event updatedEvent = new Event(
                existingEvent.getDocumentId(), existingEvent.getEventId(),
                trim(title), trim(category), trim(location),
                dateTimeMillis, existingEvent.getStatus(), capacityTotal, capacityRemaining);
