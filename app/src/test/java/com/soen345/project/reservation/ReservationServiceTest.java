...
public class ReservationServiceTest {
    ...
    @Test
    public void reserveTicket_failsOnEventUpdate_propagatesError() {
        Event event = new Event("doc1", "e1", "Title", "Cat", "Loc", 1000L, EventStatus.ACTIVE, 10, 5);
        ArgumentCaptor<ReservationRepository.ReservationActionCallback> resCallbackCaptor = ArgumentCaptor.forClass(ReservationRepository.ReservationActionCallback.class);
        ArgumentCaptor<EventActionCallback> eventCallbackCaptor = ArgumentCaptor.forClass(EventActionCallback.class);
    ...
    @Test
    public void cancelReservation_failsOnEventUpdate_propagatesError() {
        Event event = new Event("doc1", "e1", "Title", "Cat", "Loc", 1000L, EventStatus.ACTIVE, 10, 5);
        Reservation res = new Reservation("res1", "doc1", "user@test.com", 123L);
        ArgumentCaptor<ReservationRepository.ReservationActionCallback> resCallbackCaptor = ArgumentCaptor.forClass(ReservationRepository.ReservationActionCallback.class);
        ArgumentCaptor<EventActionCallback> eventCallbackCaptor = ArgumentCaptor.forClass(EventActionCallback.class);
    ...
    @Test
    public void getMyReservations_propagatesError() {
        ArgumentCaptor<ReservationRepository.ReservationListCallback> callbackCaptor = ArgumentCaptor.forClass(ReservationRepository.ReservationListCallback.class);
        TestListCallback finalCallback = new TestListCallback();

        service.getMyReservations("test@email.com", finalCallback);

        verify(reservationRepository).getReservationsForUser(eq("test@email.com"), callbackCaptor.capture());
        callbackCaptor.getValue().onError("Fetch Failed");

        assertEquals("Fetch Failed", finalCallback.error);
    }
